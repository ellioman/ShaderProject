...
public class BrightnessSaturationContrast : MonoBehaviour
{
	...
	[SerializeField] protected float brightnessAmount;
	[SerializeField] protected float saturationAmount;
	[SerializeField] protected float contrastAmount;
...
//		depthPower = Mathf.Clamp(depthPower, 0f, 5f);
	...
	protected void OnRenderImage(RenderTexture sourceTexture, RenderTexture destTexture)
	{
		if (shader != null)
		{
			...
			Mat.SetFloat("_BrightnessAmount", brightnessAmount);
			Mat.SetFloat("_SaturationAmount", saturationAmount);
			Mat.SetFloat("_ContrastAmount", contrastAmount);
